...
function AddIndividualSubmenuItems(menuName, removeCurrentMenu, showOtherMenu) {
    var orgId = localStorage.getItem("organisationId") ? +localStorage.getItem("organisationId") : 0;
    //Changes made against Issue#25
    var enumerationDate = localStorage.getItem("EnumerationDate") ? localStorage.getItem("EnumerationDate") : "";
    if (removeCurrentMenu) {
        $(".nav-sidebar").html('');
        //window.location.href = '/FacilityBasicInformation/' + orgId
    }

    //Changes made against Issue#25
    if (showOtherMenu == undefined) {
        showOtherMenu = enumerationDate == "" ? false : true;
    } 

    var linksToBeAdded = '';
    if (menuName === "Dashboard") {
        linksToBeAdded += '<li class="nav-item"><a href="/Dashboard/Facility" class="nav-link"><i class="far fa-dot-circle-o nav-icon"></i><p>Dashboard</p></a></li>';
    } else {
        linksToBeAdded += '<li class="nav-item"><a href="/Dashboard/Facility" class="nav-link"><i class="far fa-circle nav-icon"></i><p>Dashboard</p></a></li>';
    }
    if (menuName === "Profile") {
        linksToBeAdded += '<li class="nav-item"><a href="/FacilityBasicInformation/' + orgId + '" class="nav-link"><i class="far fa-dot-circle-o nav-icon"></i><p>Profile</p></a></li>';
    } else {
        linksToBeAdded += '<li class="nav-item"><a href="/FacilityBasicInformation/' + orgId + '" class="nav-link"><i class="far fa-circle nav-icon"></i><p>Profile</p></a></li>';
    }
    //Changes made against Issue#25
    if (showOtherMenu) {
        if (menuName === "Amenity") {
            linksToBeAdded += '<li class="nav-item"><a href="/Facility/AmenityOptions/' + orgId + '" class="nav-link"><i class="far fa-dot-circle-o nav-icon"></i><p>Amenity/Options</p></a></li>';
        } else {
            linksToBeAdded += '<li class="nav-item"><a href="/Facility/AmenityOptions/' + orgId + '" class="nav-link"><i class="far fa-circle nav-icon"></i><p>Amenity/Options</p></a></li>';
        }
    ...
    linksToBeAdded += '</ul>';
    $(".nav-sidebar").append(linksToBeAdded);
