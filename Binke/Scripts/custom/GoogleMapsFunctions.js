...
function initializeMap(mapElementId, resultArea) {
    var centerPoint;
    if (myLocation)
    ...
    if (resultArea.find("div.section").length > 0) {
     getUserPosition();
    }
...
function getInfoButtonHTML(mapModel) {
    ...
    var contentHtml =
        ...
        "				<h4 class='card-title'><a href='" + mapModel.profileUrl + "'>" + mapModel.name + "</a></h4>" +
        "               " + finalRatingStr + "" +
    ...
    if (mapModel.isAcceptButton) {
        var buttonString =
            ...
            "				<a href='" + mapModel.profileUrl + "' class='btn btn-primary'>Accepting New Patient</a>" +
            "			</div>";
